...
[XamlFilePath("Views/StepDataView.xaml")]
public class StepDataView : ContentPage
{
	...
	private void InitializeComponent()
	{
		...
		{
			AssemblyName = typeof(StepDataView).GetTypeInfo().Assembly.GetName(),
			ResourcePath = "Views/StepDataView.xaml",
			Instance = this
		}))
		{
		...
		Grid grid = new Grid();
		StepDataView stepDataView;
		NameScope nameScope = (NameScope)(NameScope.GetNameScope(stepDataView = this) ?? new NameScope());
		NameScope.SetNameScope(stepDataView, nameScope);
		...
		this.grid = flexGrid;
		stepDataView.SetValue(NavigationPage.HasBackButtonProperty, true);
		stepDataView.SetValue(NavigationPage.BackButtonTitleProperty, "戻る");
		stepDataView.SetValue(Page.TitleProperty, "データ表");
		grid.SetValue(View.VerticalOptionsProperty, LayoutOptions.FillAndExpand);
		flexGrid.SetValue(Grid.RowProperty, 0);
		bindingExtension.Path = "DisplayStepData";
		...
		gridColumn2.Binding = "Step";
		gridColumn2.Header = "歩数";
		gridColumn2.Format = "#,##0";
		...
		VisualDiagnostics.RegisterSourceInfo(flexGrid, new Uri("Views/StepDataView.xaml", UriKind.RelativeOrAbsolute), 16, 10);
		stepDataView.SetValue(ContentPage.ContentProperty, grid);
		VisualDiagnostics.RegisterSourceInfo(grid, new Uri("Views/StepDataView.xaml", UriKind.RelativeOrAbsolute), 15, 6);
		VisualDiagnostics.RegisterSourceInfo(stepDataView, new Uri("Views/StepDataView.xaml", UriKind.RelativeOrAbsolute), 2, 2);
	}

	private void __InitComponentRuntime()
	{
		this.LoadFromXaml(typeof(StepDataView));
