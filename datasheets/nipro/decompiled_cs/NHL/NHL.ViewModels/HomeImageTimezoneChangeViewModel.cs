...
public class HomeImageTimezoneChangeViewModel : ViewModelBase
{
	...
	public async Task Save()
	{
		await ExecAsync(async delegate
		{
			...
			{
				Log.Info("【IG】【HomeImageTimezoneChangeViewModel】【Save】Sync start");
				await PhotographService.Sync();
			}).ConfigureAwait(continueOnCapturedContext: false);
		});
		base.EventAggregator.PublishOnUIThread(new HomeRegistDateSaveEvent());
	}
