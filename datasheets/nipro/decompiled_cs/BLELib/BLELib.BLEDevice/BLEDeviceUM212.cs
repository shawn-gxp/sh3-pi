...
public class BLEDeviceUM212 : IBLEDevice
{
	...
	private static IBluetoothLE _CrossBluetoothLE = CrossBluetoothLE.Current;
	...
	{
		Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【_IAdapter_DeviceConnectionLost】Device.Name={0}, Device.Id={1}", new object[2]
		{
			e.Device?.Name,
			e.Device?.Id
		}));
	...
	public async Task<IList<string>> Pairing(IList<string> param = null)
	{
		...
		try
		{
			...
			Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【Pairing】ConnectToDeviceAsync:Device.Name={0}, Device.Id={1}", new object[2]
			{
				Device?.Name,
				Device?.Id
			}));
			...
			{
				Log.Info("【BLELib】【BLEDeviceUM212】【Pairing】scan already processing, stop scan.");
			...
			Task pairingTask = Task.Run(async delegate
			{
				...
				{
					Log.Info("【BLELib】【BLEDeviceUM212】【Pairing】CancelToken was cancelled, finish pairing.");
				}
				else
				{
					...
					Log.Info("【BLELib】【BLEDeviceUM212】【Pairing】serial number ReadAsync:Service=" + service.Id.ToString() + ", Characteristic=" + characteristic.Id.ToString());
					DeviceBondedEvent.Wait();
					byte[] array = await characteristic.ReadAsync();
					serialNumber = ToAsciiStringFromBytes(array);
					Log.Info("【BLELib】【BLEDeviceUM212】【Pairing】serial number WriteAsync:Result=" + BitConverter.ToString(array) + ":SerialNumber=" + serialNumber);
				}
			});
			if (await Task.WhenAny(pairingTask, Task.Delay(PAIRING_REQUEST_TIMEOUT)) != pairingTask)
			{
				Log.Info("【BLELib】【BLEDeviceUM212】【Pairing】ペアリング失敗：PAIR_TIMEOUT");
		...
		{
			Log.Warn($"【BLELib】【BLEDeviceUM212】【Pairing】TaskCanceledException発生：{ex}");
			goto IL_05e8;
		}
		catch (OperationCanceledException ex2)
		{
			Log.Warn($"【BLELib】【BLEDeviceUM212】【Pairing】OperationCanceledException発生：{ex2}");
			goto IL_05e8;
		}
		catch (Exception ex3)
		{
			Log.Error($"【BLELib】【BLEDeviceUM212】【Pairing】例外発生：{ex3}");
		...
		finally
		{
			...
			Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【Pairing】DisconnectDeviceAsync:Device.Name={0}, Device.Id={1}", new object[2]
			{
				Device?.Name,
				Device?.Id
			}));
	...
	public async Task<object> ReciveStart(Action<int, int> handler, int timeout, IList<string> param = null)
	{
		...
		try
		{
			...
			_IAdapter.DeviceConnected += _IAdapter_DeviceConnected;
			_CancelToken = new CancellationTokenSource();
			Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【ReciveStart】ConnectToDeviceAsync:Device.Name={0}, Device.Id={1}", new object[2]
			{
				Device?.Name,
				Device?.Id
			}));
			...
			Task receiveTask = Task.Run(async delegate
			{
				...
				Log.Info("【BLELib】【BLEDeviceUM212】【ReciveStart】メーター日時 WriteAsync:Service=" + service.Id.ToString() + ", Characteristic=" + characteristic.Id.ToString() + ", data=" + BitConverter.ToString(datetimeArray));
				await Task.Delay(1000);
				await MainThread.InvokeOnMainThreadAsync(async () => await characteristic.WriteAsync(datetimeArray));
				Log.Info("【BLELib】【BLEDeviceUM212】【ReciveStart】メーター日時 ReadAsync:Service=" + service.Id.ToString() + ", Characteristic=" + characteristic.Id.ToString());
				byte[] array = await characteristic.ReadAsync();
				Log.Info("【BLELib】【BLEDeviceUM212】【ReciveStart】メーター日時 ReadAsync:Result=" + BitConverter.ToString(array));
			}, _CancelToken.Token);
			if (await Task.WhenAny(receiveTask, Task.Delay(timeout)) != receiveTask)
			{
				Log.Info("【BLELib】【BLEDeviceUM212】【RceiveStart】ReceiveStart失敗：RCV_TIMEOUT");
		...
		{
			Log.Warn($"【BLELib】【BLEDeviceUM212】【ReciveStart】TaskCanceledException発生：{ex}");
			goto IL_073a;
		}
		catch (OperationCanceledException ex2)
		{
			Log.Warn($"【BLELib】【BLEDeviceUM212】【ReciveStart】OperationCanceledException発生：{ex2}");
			goto IL_073a;
		}
		catch (Exception ex3)
		{
			Log.Error($"【BLELib】【BLEDeviceUM212】【ReciveStart】例外発生：{ex3}");
		...
		finally
		{
			...
			if (Device != null)
			{
				Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【ReciveStart】DisconnectDeviceAsync:Device.Name={0}, Device.Id={1}", new object[2]
				{
					Device?.Name,
					Device?.Id
				}));
	...
	{
		Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【_IAdapter_DeviceConnected】Device.Name={0}, Device.Id={1}", new object[2]
		{
			e.Device?.Name,
			e.Device?.Id
		}));
	...
	public async Task ReceiveStop()
	{
		...
		if (Device != null)
		{
			Log.Info(string.Format("【BLELib】【BLEDeviceUM212】【ReceiveStop】DisconnectDeviceAsync:Device.Name={0}, Device.Id={1}", new object[2]
			{
				Device?.Name,
				Device?.Id
			}));
	...
	public void Cancel()
	{
		if (_CancelToken != null)
		{
			Log.Info("【BLELib】【BLEDeviceUM212】【Cancel】Cancel all tasks.");
			_CancelToken.Cancel();
		}
	}
	...
	private void BloodPressureMeasurement_CharacteristicValueUpdateHandler(object sender, CharacteristicUpdatedEventArgs e)
	{
		try
		{
			...
			Log.Info("【BLELib】【BLEDeviceUM212】【BloodPressureMeasurement_CharacteristicValueUpdateHandler】CharacteristicValueUpdateHandle:sender=" + (sender as ICharacteristic).Uuid + ", buffer=" + BitConverter.ToString(e.Characteristic.Value));
