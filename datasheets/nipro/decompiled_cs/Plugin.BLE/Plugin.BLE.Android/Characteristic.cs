...
public class Characteristic : CharacteristicBase
{
	...
	public override CharacteristicPropertyType Properties => (CharacteristicPropertyType)_nativeCharacteristic.Properties;

	public override event EventHandler<CharacteristicUpdatedEventArgs> ValueUpdated;

	public Characteristic(BluetoothGattCharacteristic nativeCharacteristic, BluetoothGatt gatt, IGattCallback gattCallback, IService service)
		: base(service)
	{
	...
	{
		return Task.FromResult((IList<IDescriptor>)_nativeCharacteristic.Descriptors.Select((BluetoothGattDescriptor item) => new Descriptor(item, _gatt, _gattCallback, this)).Cast<IDescriptor>().ToList());
	}

	protected override async Task<byte[]> ReadNativeAsync()
	{
		return await TaskBuilder.FromEvent<byte[], EventHandler<CharacteristicReadCallbackEventArgs>, EventHandler>(ReadInternal, (Action<byte[]> complete, Action<Exception> reject) => delegate(object sender, CharacteristicReadCallbackEventArgs args)
		{
			if (args.Characteristic.Uuid == _nativeCharacteristic.Uuid)
			{
				complete(args.Characteristic.GetValue());
			}
		}, delegate(EventHandler<CharacteristicReadCallbackEventArgs> handler)
		{
			_gattCallback.CharacteristicValueUpdated += handler;
		}, delegate(EventHandler<CharacteristicReadCallbackEventArgs> handler)
		{
			_gattCallback.CharacteristicValueUpdated -= handler;
		}, (Action<Exception> reject) => delegate
		{
			reject(new Exception($"Device '{base.Service.Device.Id}' disconnected while reading characteristic with {Id}."));
	...
	protected override async Task<bool> WriteNativeAsync(byte[] data, CharacteristicWriteType writeType)
	{
		...
		return await TaskBuilder.FromEvent<bool, EventHandler<CharacteristicWriteCallbackEventArgs>, EventHandler>(delegate
		{
			InternalWrite(data);
		}, (Action<bool> complete, Action<Exception> reject) => delegate(object sender, CharacteristicWriteCallbackEventArgs args)
		{
			if (args.Characteristic.Uuid == _nativeCharacteristic.Uuid)
			{
				complete(args.Exception == null);
			}
		}, delegate(EventHandler<CharacteristicWriteCallbackEventArgs> handler)
		{
			_gattCallback.CharacteristicValueWritten += handler;
		}, delegate(EventHandler<CharacteristicWriteCallbackEventArgs> handler)
		{
			_gattCallback.CharacteristicValueWritten -= handler;
		}, (Action<Exception> reject) => delegate
		{
			reject(new Exception($"Device '{base.Service.Device.Id}' disconnected while writing characteristic with {Id}."));
		}, delegate(EventHandler handler)
		{
			_gattCallback.ConnectionInterrupted += handler;
		}, delegate(EventHandler handler)
	...
	protected override async Task StartUpdatesNativeAsync()
	{
		...
		if (!_gatt.SetCharacteristicNotification(_nativeCharacteristic, enable: true))
		{
		...
		{
			IList<IDescriptor> source = await GetDescriptorsAsync();
			IDescriptor descriptor = source.FirstOrDefault((IDescriptor d) => d.Id.Equals(ClientCharacteristicConfigurationDescriptorId)) ?? source.FirstOrDefault();
	...
	{
		_gattCallback.CharacteristicValueUpdated -= OnCharacteristicValueChanged;
		bool flag = _gatt.SetCharacteristicNotification(_nativeCharacteristic, enable: false);
		Trace.Message("Characteristic.StopUpdatesNative, successful: {0}", flag);
		if (!flag)
		{
