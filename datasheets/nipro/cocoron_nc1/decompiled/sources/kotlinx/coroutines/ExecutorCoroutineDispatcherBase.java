...
public abstract class ExecutorCoroutineDispatcherBase extends ExecutorCoroutineDispatcher implements Delay {
    ...
    public void mo1372dispatch(CoroutineContext context, Runnable block) {
        ...
        try {
            ...
            Dispatchers.getIO().mo1372dispatch(context, block);
        }
    }
    ...
    public void mo1373scheduleResumeAfterDelay(long timeMillis, CancellableContinuation<? super Unit> continuation) {
        ScheduledFuture<?> scheduleBlock = this.removesFutureOnCancellation ? scheduleBlock(new ResumeUndispatchedRunnable(this, continuation), continuation.getContext(), timeMillis) : null;
        if (scheduleBlock != null) {
            JobKt.cancelFutureOnCancellation(continuation, scheduleBlock);
        } else {
            DefaultExecutor.INSTANCE.mo1373scheduleResumeAfterDelay(timeMillis, continuation);
        }
    }

    @Override // kotlinx.coroutines.Delay
    public DisposableHandle invokeOnTimeout(long timeMillis, Runnable block, CoroutineContext context) {
        ScheduledFuture<?> scheduleBlock = this.removesFutureOnCancellation ? scheduleBlock(block, context, timeMillis) : null;
        if (scheduleBlock != null) {
