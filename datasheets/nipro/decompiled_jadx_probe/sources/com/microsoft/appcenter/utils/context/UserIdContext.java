    ...
    public static boolean checkUserIdValidForOneCollector(String str) {
        ...
        if (indexOf >= 0) {
            ...
            if (!substring.equals(CUSTOM_PREFIX)) {
                AppCenterLog.error("AppCenter", String.format("userId prefix must be '%s%s', '%s%s' is not supported.", CUSTOM_PREFIX, Constants.COMMON_SCHEMA_PREFIX_SEPARATOR, substring, Constants.COMMON_SCHEMA_PREFIX_SEPARATOR));
                return false;
            }
